...
async function loadLedger() {
  ...
  const [playersRes, sessionsRes, gamesRes, settlementsRes] = await Promise.all(
    [
  ...
  const grouped: SessionWithGames[] = allSessions.map((session) => ({
    ...
    settlements: allSettlements.filter((s) => s.session_id === session.id),
  }));

  return { players: allPlayers, sessionsData: grouped };
}

function computeOverallStats(
  sessionsData: SessionWithGames[],
  players: Player[]
): OverallPlayerStats[] {
  ...
  for (const sd of sessionsData) {
    ...
    const hasUnsettled =
      sd.settlements.length === 0 || sd.settlements.some((s) => !s.settled);
...
export default async function LedgerPage() {
  ...
  const overallStats = computeOverallStats(sessionsData, players);

  const overallSettlements = calculateSettlements(
    overallStats
